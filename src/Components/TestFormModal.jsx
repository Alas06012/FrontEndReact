...
const TestFormModal = ({ isOpen, onClose, testData, onSubmit, initialTime, testStarted }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!isComplete) {
            const result = await Alert({
                ...
                text: '¿Estás seguro que deseas enviar el test?',
                icon: 'warning',
                type: 'confirm',
        ...
        // Si está completo o el usuario confirmó:
        onSubmit({
    ...
    return (
        ...
        >
            ...
            {(() => {
                const enhancedTitles = useMemo(() => {
                    let counter = 1;
                    return allTitlesWithSection.map(title => ({
                        ...title,
                        questions: title.questions.map(q => ({
                            ...q,
                            globalIndex: counter++,
